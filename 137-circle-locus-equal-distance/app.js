...
(function () {
    ...
    function classifyPoint(h, k, r, x, y) {
        ...
        const rSq = r * r;
        const tol = 0.05 * Math.max(1, r); // Floating point numerical tolerance

        if (Math.abs(dist - r) < tol || Math.abs(distSq - rSq) < (0.1 * rSq)) {
    ...
    function updatePredictAndCheck() {
        ...
        if (diff < 0.1 * rightVal || Math.abs(Math.sqrt(leftVal) - r) < 0.05 * r) {
    ...
    function updateProof() {
        ...
        const relSymbol = Math.abs(cp - r) < 0.05 ? '=' : (cp < r ? '&lt;' : '&gt;');
    ...
    function setupEventListeners() {
        ...
        // ---------------------------------------------------------
        // 10. INTERACTIVE SVG MOUSE / TOUCH DRAG HANDLERS
        // ---------------------------------------------------------
        ...
        function handlePointerDown(e) {
            ...
            // Check if clicking near Point P
        ...
        function handlePointerMove(e) {
            ...
            if (isDraggingP) {
                if (state.mode === 'locus') {
                    // Lock P to circle perimeter angle
                    const angle = Math.atan2(mouseMath.y - state.center.k, mouseMath.x - state.center.h);
                    state.point.x = parseFloat((state.center.h + state.radius * Math.cos(angle)).toFixed(2));
                    state.point.y = parseFloat((state.center.k + state.radius * Math.sin(angle)).toFixed(2));
                } else {
                    // Free dragging
